...
const foreignLane = document.getElementById('foreign-lane');
...
function stackedRow(item, index) {
  return (item.row ?? index % 3) * 50 + 18;
}
...
function renderLane(data, laneEl) {
  data.forEach((king, index) => {
    ...
    bar.style.top = `${stackedRow(king, index)}px`;
    bar.innerText = king.name;

    bar.addEventListener('mousemove', (event) => {
      const contextLine = king.empire
        ? `제국/구분: ${king.empire}<br>등장/관련 본문: ${king.books}<br>`
...
renderLane(foreignKings, foreignLane);
